...
RUN apk --update --no-cache add \
  git openssl~=1.1.1s-r0 jq tshark python3-dev py3-pip bash make curl gcc make g++ zlib-dev talloc-dev libressl openssl-dev linux-headers

RUN wget https://github.com/FreeRADIUS/freeradius-server/archive/release_3_2_1.tar.gz \
  && tar xzvf release_3_2_1.tar.gz \
  && cd freeradius-server-release_3_2_1 \
